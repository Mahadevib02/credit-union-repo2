...
public class CreditUnionDemo {
		...
		while(true) {
			...
			for(; i<bankAccounts.length; i++) {
				if(accountNumber==bankAccounts[i].getAccountNumber()) {
					...
					if(bankAccounts[i].getAccountType().equals(AccountType.CERTIFICATE_OF_DEPOSITE)) {
						...
						if(trans.equalsIgnoreCase("d")) {
							...
							Double amount= Double.parseDouble(scanner.nextLine());
							if(bankAccounts[i] instanceof ConsumerAccount) {
								((ConsumerAccount)bankAccounts[i] ).withdraw(amount);
							}else if(bankAccounts[i] instanceof CommercialAccount)  {
								((CommercialAccount)bankAccounts[i] ).withdraw(amount);
							}else {
								System.out.println("Invalid account");
							//TO DO
						}
	...
	private static void populateAccounts(Account[] accounts) {
		...
		accounts[3]= new CertificateOfDepositAccount(808765456L,AccountType.CERTIFICATE_OF_DEPOSITE,
							"Rohit Sharma",LocalDate.of(2012, 10, 5), 500000.0,LocalDate.of(2015, 10, 5));
		accounts[4]= new CertificateOfDepositAccount(805705000L,AccountType.CERTIFICATE_OF_DEPOSITE,
